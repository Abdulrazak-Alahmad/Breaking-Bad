...
export default function CharacterDetails(productsList) {
    const { characterId } = useParams()
    const character = useFetch(`/api/characters/${characterId}`, [characterId])
    const { LydiaImage, HollyImage } = useContext(FavouritesContext)
    return (
        <div>
            <Header title={character.isLoading ?
                <RotatingLines
                    strokeColor="grey"
                    strokeWidth="5"
                        ...
                        <>
                            ...
                            <div className=' characterDetailsInfo'>
                                ...
                                <div className='characterDetailsInfoChild'>
                                    ...
                                    <b> Quotes:</b> <Link className='link' to={`/api/characterQuotes/${character.data[0].name.split(" ")[0]}+${character.data[0].name.split(" ")[1]}`}>Click here</Link>
                                </div>
                            </div>
                        </>
            }
